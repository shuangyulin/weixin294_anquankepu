<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="科普标题" prop="kepubiaoti">
          <el-input v-model="ruleForm.kepubiaoti" 
              placeholder="科普标题" clearable  :readonly="ro.kepubiaoti"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="科普标题" prop="kepubiaoti">
              <el-input v-model="ruleForm.kepubiaoti" 
                placeholder="科普标题" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
      ...
      <el-col :span="12">
        ...
        <div v-else>
          <el-form-item class="input" label="知识分类" prop="zhishifenlei">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.kepushipin" label="科普视频" prop="kepushipin">
          <file-upload
          tip="点击上传科普视频"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.kepushipin?ruleForm.kepushipin:''"
          @change="kepushipinUploadChange"
          ></file-upload>
        </el-form-item> 
        <div v-else>
          <el-form-item v-if="ruleForm.kepushipin" label="科普视频" prop="kepushipin">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.kepushipin)">预览</el-button>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="科普内容" prop="kepuneirong">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.kepuneirong" label="科普内容" prop="kepuneirong">
                    <span v-html="ruleForm.kepuneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhishifenlei = true;
          ...
          if(o=='kepushipin'){
            this.ruleForm.kepushipin = obj[o];
	    this.ro.kepushipin = true;
            continue;
          }
          if(o=='kepuneirong'){
	...
	this.ruleForm.kepuneirong = this.ruleForm.kepuneirong.replace(reg,'../../../springbootsme07/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.kepushipin!=null) {
		this.ruleForm.kepushipin = this.ruleForm.kepushipin.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kepuzhishi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kepuzhishiCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    kepushipinUploadChange(fileUrls) {
	this.ruleForm.kepushipin = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
