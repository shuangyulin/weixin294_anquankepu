	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussanlifenxi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussanlifenxi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussanlifenxi`, this.ruleForm);
						}else{
							await this.$api.add(`discussanlifenxi`, this.ruleForm);
						}
						uni.setStorageSync('discussanlifenxiCleanType',true);
